...
From the formula provided by the nutrition team, we used the technically correct data to calculate the Framingham score. The Framingham score is a cardiovascular disease risk assessment tool. The calculation includes an individual's age, sex, total cholesterol level, HDL cholesterol level, systolic blood pressure, smoking status and siabetes status. Since the data available in our dataset is in terms of ranges, we cannot calculate the score directly. 
...
Moreover, as there exist class imbalance due to majority of younger groups being in the low risk class, we only included people with age greater than 45. This provides a better class balance that prevents future modelling to be unreliable and innacurate. Also, this chosen age limit follows the nutritional theory where this is the recommended age for the first heart health check.
...
Next, we visualise the skewness of the data to make sure that it satisfies model assumptions. From the graph, we can see there are many variables that are heavily skewed. Hence, we take the log transformation of these numerical variables to obtain Gaussian distributions and to allow us standardise the values to bring them onto the same scale. This is done to ensure the model performs well and uses the information to the most. Highly correlated variables are also removed as it will bias the outcome by causing multicollinearity.
```{r}
...
By completing the processing of data, we keep all the variables required for easy reference later on.
```{r}
...
The boxplot of the accuracy of each model is plotted and compared. The random forest and decision tree algorithms with feature selection seem to work best. 

Then, we saved the accuracy values of these different models and the two chosen models separately as tables.
```{r}
# load file containing scores from 10-fold CV 
load("10_fold_res.Rdata")

# boxplot of different models' accuracy
boxplot(full_knn, full_svm, full_rf, full_tree, boruta_knn, boruta_svm, boruta_rf, boruta_tree, fra_knn, fra_svm, fra_rf, fra_tree, names = c("knn", "svm", "rf", "tree", "bknn", "bsvm", "brf", "btree", "fra_knn", "fra_svm", "fra_rf", "fra_tree"), horizontal = TRUE, xlab = "Accuracy")
...
Given that the accuracy of the models are not excellent, we also tried other versions of the two models: random forest and desion tree. The three versions are differentiated by the dataset applied to the models. We tried the performances with dataset containing only age, sex, smoking status and diabetes status, then with full dataset containing age, sex, status variables, diet and lifestyle variables, lastly with dataset containing variables selected by Boruta feature selection. A graph of boxplots is also drawn to compare the accuracies.

Decision tree appears to perform the best. Thus, it is selected as the final model. 
```{r}
...
boxplot(full_rf, full_tree, boruta_rf, boruta_tree, fra_rf, fra_tree, names = c("rf", "tree", "brf", "btree", "frf", "ftree"), horizontal = TRUE, xlab = "Accuracy")
```
...
## Asides
This is a version where we renamed the variables to be more understandable. It can then be used for better visualisation and tabulation of data.
```{r}
...
clearer_names <- rename(tree_variables, c("BMI" = "BMISC", "Age" = "AGEC", "Waist_circumference" = "PHDCMWBC", "Equivalised_Income(deciles)" = "INCDEC", "Whether_has_diabetes_mellitus" = "DIABBC", "Sex" = "SEX", "Daily_smoker_status" = "SMKDAILY", "Smoker_status" = "SMKSTAT", "Protein" = "PROTEIN", "Dietary_Fibre" = "FIBRE", "Pro-vitamin_A" = "PROVITA", "Retinol_equivalents" = "RETEQV", "Thiamin_(B1)" = "B1MG", "Riboflavin_(B2)" = "B2MG", "Niacin (B3)" = "B3MG", "Niacin_equivalents" = "NIACINEQ", "Calcium" = "CALMG", "Iron" = "IRONMG", "Magnesium" = "MAGMG", "Phosphorous" = "PHOSMG", "Potassium" = "POTASMG", "Selenium" = "SELUG", "Zinc" = "ZINCMG", "Saturated_fat" = "SFAG", "Monounsaturated_fat" = "MUFAG", "Linoleic_acid" = "LAG"))
```

To get a better idea of the process, the model and the variables, we have done several visualisations which are included in the folder named **figures**. We have not included the code in this file as we want to avoid having redundant information and the figures are already saved into the folder. We have drawn a flowchart to present the whole process of our analysis. Then, the tables of variables selected and their importance are plotted to have a better understanding of how they impact the response variable. A confusion matrix is also plotted to show how well the model predicts each class. We have also included a **vis.ipynb** file in the figures folder that include visualisations done with the help of python.
