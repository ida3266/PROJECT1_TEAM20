...
# saving the accuracy values from 10-fold cross validation for different models
...
```{r}
boruta <- Boruta(cvd_class~., data = biom_food_scaled, maxRuns = 500, doTrace = 1)
boruta_fixed <- TentativeRoughFix(boruta)
boruta_vars <- getSelectedAttributes(boruta_fixed)
```
...
# build decision tree
tree_model <- rpart(cvd_class ~., data = biom_food_scaled %>% select(boruta_vars, cvd_class), method = "class", model = TRUE, control = rpart.control(cp=0.005))
